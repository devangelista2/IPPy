...
def BaselineModel(image_shape, n_ch, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[], dw_expand=1, ffn_expand=2):
    ...
    # Synthesis
    encs = encs[::-1]
    for i, n_conv in enumerate(dec_blk_nums):    
...
def NAFModel(image_shape, n_ch, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[], dw_expand=1, ffn_expand=2):
    # Get the shape
    H, W, C = image_shape
    ...
    for n_conv in enc_blk_nums:
        for _ in range(n_conv):
            h = _blocks.NAFBlock(h, n_ch, dw_expand, ffn_expand)
        encs.append(h)
        h = ks.layers.Conv2D(n_ch * 2, kernel_size=2, strides=2)(h)
